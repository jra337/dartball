/*
// Decrypt with:
// echo "U2FsdGVkX19qQIRedr5nh5AZVM00gEqFGgGpykFXH8E=" | openssl aes-256-cbc -d -salt -pbkdf2 -iter 10000 -pass pass:"password" -base64
const encrypt = async (str, password, pbkdf2iterations = 10000) => {
...
// Encrypt with:
// echo "test test test" | openssl aes-256-cbc -e -salt -pbkdf2 -iter 10000 -pass pass:"password" -base64
const decrypt = async (base64, password, pbkdf2iterations = 10000) => {
  const encoder = new TextEncoder();
...
encrypt('test123', 'password').then(encrypted => {
  console.log(encrypted)
  return decrypt(encrypted, 'password')
}).then(decrypted => {
  console.log(decrypted)
  return decrypt('U2FsdGVkX1+QDemu+6vhmreqXfwGygk6SksaTtOVuUk=', 'password')
}).then(decrypted => {
  console.log(decrypted)
});
*/

const decrypt = async (buffer, password, pbkdf2iterations = 10000) => {
